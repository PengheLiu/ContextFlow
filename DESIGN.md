...
4. **嵌套 `createDocWithMd` 会隐式创建缺失父文档，且最终路径创建不是幂等。**
   文件树会立即看见新文档，但 `blocks` SQL 索引有延迟。若先创建 `/日期/文章`，
   再用 SQL 判断 `/日期` 是否存在并创建日报，就可能得到两个同名日期文档。
   因此 ContextFlow 先用 `getIDsByHPath` 解析/显式创建日期文档并写入 `idxdoc`，
   再用确定的 `parentID` 创建文章。若历史上已有重复 hpath，只稳定选主并告警，
   不自动删除或合并用户内容。
